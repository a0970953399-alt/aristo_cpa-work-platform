...
- 2026-09-09 在 `TimesheetView.tsx` 新增「補登工時」功能，僅老闆與主管看得到入口並可操作。
- 補登人員選單只顯示姓名，不依角色分組或標示角色；名單包含所有啟用中的主管、工讀生與實習生等平台人員，並排除所有老闆帳號。
- 補登欄位包含日期、上下班時間、休息時數、系統計算的實際工時、補登原因與補充說明。不能補登 `2026-06-30` 以前的已結算期間、未來日期，或同一人同一天的第二筆紀錄。
- 補登紀錄保存補登人員、時間、原因與說明；工時列表以「補登」標記呈現，游標停留可查看稽核資訊。補登來源欄位在後續編輯時不可竄改。
- `firestore.rules` 已同步限制：只有老闆、主管能替他人補登，建立對象必須是啟用中的非老闆人員；一般使用者仍只能建立自己的正常打卡紀錄，也不能偽造補登欄位。
- 前端 `npm.cmd run build` 已成功；Firebase Rules `--dry-run` 編譯成功。Rules 尚未部署到正式 Firebase，因此正式站補登功能須待 Rules 部署後才能成功寫入。

### 記帳工作單簽名

- 2026-09-09 將客戶「記帳工作單」工作紀錄中的簽名名稱改為中文：Incharge 顯示「周榆」，會計師顯示「鄧會」。
- `ClientMasterView.tsx` 的畫面與 Word 匯出共用 `getResolvedWorkRecord`，因此兩處會同步套用新名稱。
- 已兼容資料庫中既有的 `Yue`、`Yeu` 與 `BD` 簽名，不必遷移或重寫正式資料；舊紀錄在畫面及之後新輸出的 Word 中會轉為「周榆」與「鄧會」，日期部分保持不變。
