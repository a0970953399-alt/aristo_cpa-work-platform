...
- `TimesheetView.tsx` 以顏色標示已結算工時，並在熱力圖與列表列背景使用同一套結算色；未結算仍維持原本顯示。
- Firestore Rules 已限制一般員工不能寫入結算欄位，主管不能結算，已結算或歷史工時不能再被前端更新或刪除。
- 新增本機工時測試模式：在網址加上 `?timesheetTest=1` 會進入 `TimesheetTestMode.tsx`，使用假老闆、假主管、假工讀生/實習生與假工時資料；測試模式不訂閱 Firestore，也不寫入正式 Firebase。
