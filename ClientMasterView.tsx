        ...
        reader.onload = async (event) => {
            try {
                ...
                const newClients: Client[] = json.map((row) => ({
                    ...
                    code: String(row['客戶代號'] || ''),
                    year: String(row['記帳年度'] || ''),
                    workNo: String(row['記帳工作'] || ''),
                    name: String(row['公司簡稱'] || row['公司名稱'] || ''),
                    fullName: String(row['公司名稱'] || ''),
                    taxId: String(row['統編'] || ''),
                    ...
                    regAddress: String(row['登記地址'] || ''),
                    contactAddress: String(row['聯絡地址'] || ''),
                    cpa: String(row['負責會計師'] || ''),
                    period: String(row['委任期限'] || ''),
                    feeMonthly: String(row['每月公費'] || ''),
                    feeWithholding: String(row['各類扣繳'] || ''),
                    feeTax: String(row['結算申報'] || ''),
                    fee22_1: String(row['22-1申報'] || ''),
                    // 勾選判斷：填入 V 或 1 代表勾選
                    chkAccount: row['會計帳務'] === 'V' || row['會計帳務'] === 1,
                    chkInvoice: row['買發票'] === 'V' || row['買發票'] === 1,
                    chkVat: row['營業稅'] === 'V' || row['營業稅'] === 1,
                    chkWithholding: row['扣繳申報'] === 'V' || row['扣繳申報'] === 1,
                    chkHealth: row['補充保費'] === 'V' || row['補充保費'] === 1,
                    boxReview: false,
                    boxAudit: false,
                    boxCpa: false,
                }));

                if (window.confirm(`偵測到 ${newClients.length} 筆客戶，是否確定匯入並儲存？`)) {
                    // 與現有資料合併並寫入 TaskService (實現連動)
                    const combined = [...clients, ...newClients];
                    await TaskService.saveClients(combined);
                    onUpdate(); // 通知 Dashboard 更新
                    alert("🎉 匯入並儲存成功！主管那邊也會同步看到資料。");
                }
            } catch (err) {
            ...
            <div className="bg-white shadow-sm p-4 flex justify-between items-center z-10">
                ...
                <div className="flex gap-2">
                    <button 
                        ...
                        className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors font-bold"
                    >
                        📊 匯入 Excel
                    </button>
                    <input 
            ...
            {selectedClient && (
                <div className="fixed inset-0 bg-black/60 z-[110] flex items-center justify-center p-4" onClick={() => setSelectedClient(null)}>
                    <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                        {/* 內容省略，維持您原本的表單結構... */}
                        {/* (此處包含您原本檔案中所有的 input 和 checkbox 邏輯) */}
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                             {/* ... 您原本的詳細表單 UI ... */}
                             {/* (此處請保留您原本程式碼中 grid grid-cols-1 md:grid-cols-2 那一整段) */}
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-3 gap-3 bg-indigo-50 p-3 rounded-lg border border-indigo-100">
                                        <div><label className="text-xs text-indigo-800 font-bold">客戶編號</label><input type="text" value={selectedClient.code || ''} onChange={e => handleChange('code', e.target.value)} className="w-full border p-2 rounded-lg font-mono" /></div>
                                        <div><label className="text-xs text-indigo-800 font-bold">年度</label><input type="text" value={selectedClient.year || ''} onChange={e => handleChange('year', e.target.value)} className="w-full border p-2 rounded-lg font-mono" /></div>
                                        <div><label className="text-xs text-indigo-800 font-bold">記帳工作</label><input type="text" value={selectedClient.workNo || ''} onChange={e => handleChange('workNo', e.target.value)} className="w-full border p-2 rounded-lg font-mono" /></div>
                                    </div>
                                    <div><label className="text-xs text-gray-500 font-bold">公司全名</label><input type="text" value={selectedClient.fullName || ''} onChange={e => handleChange('fullName', e.target.value)} className="w-full border p-2 rounded-lg" /></div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div><label className="text-xs text-gray-500 font-bold">每月公費</label><input type="text" value={selectedClient.feeMonthly || ''} onChange={e => handleChange('feeMonthly', e.target.value)} className="w-full border p-2 rounded-lg text-blue-600 font-bold" /></div>
                                        <div><label className="text-xs text-gray-500 font-bold">各類扣繳 (f1)</label><input type="text" value={selectedClient.feeWithholding || ''} onChange={e => handleChange('feeWithholding', e.target.value)} className="w-full border p-2 rounded-lg text-blue-600 font-bold" /></div>
                                    </div>
                        ...
                        <div className="p-4 bg-gray-50 border-t rounded-b-3xl flex justify-between items-center">
                            <button onClick={handleSave} disabled={isSaving} className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg">
                                {isSaving ? '儲存中...' : '💾 儲存資料'}
                            </button>
                            <button onClick={handleGenerateWord} className="px-6 py-3 bg-black text-white font-bold rounded-xl hover:bg-gray-800 shadow-lg">
                                🖨️ 生成記帳工作單
                            </button>
                        </div>
                    </div>
